...
export class ChartLoader {
  ...
  private static parseChart(data: any): ChartData {
    ...
    // 根据难度限制音符数量
    const maxNotesMap: Record<string, number> = {
      'easy': 200,
      'normal': 300,
      'hard': 500,
      'insane': 1000
    ...
    const maxNotes = maxNotesMap[chart.metadata.difficulty] || 300;
    if (chart.notes.length > maxNotes) {
      console.warn(`Chart has ${chart.notes.length} notes, limiting to ${maxNotes} for ${chart.metadata.difficulty} difficulty`);
      chart.notes = chart.notes.slice(0, maxNotes);
    }
